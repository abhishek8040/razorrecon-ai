...
engine_path = "backend/app/engine.py"
...
class ReconciliationEngine:
    ...
    def run(self, merchant_id: str = None):
        ...
        run.auto_matched = self.session.exec(select(ReconciliationResult).where(ReconciliationResult.run_id == self.run_id, ReconciliationResult.decision_source == "DETERMINISTIC", ReconciliationResult.result_type == "MATCHED_EXACT")).all().__len__()
            ...
            for s_id, s in unmatched_settlements.items():
                ...
                if s.reference == p.id and s.settlement_amount == p.amount:
                    ...
                    for b_id, b in unmatched_bank_txs.items():
                        ...
                        if b.reference == s.id and b.amount == s.settlement_amount:
                            b_match = b
                            break
        ...
        for p_id, p in unmatched_payments.items():
            ...
            if not resolved:
                ...
                exc = ExceptionRecord(
                    ...
                    description=f"Could not automatically reconcile payment {p.id}. Candidates found: {len(candidates)}. Reason: why_not_auto_resolved",
                    status="OPEN"
                )

...
def test_ai_investigate_fallback(client: TestClient, session: Session, monkeypatch):
    ...
    # It will use the MockAIProvider
    response = client.post(f"/api/exceptions/{exc.id}/investigate")
    assert response.status_code == 200
    data = response.json()
    assert data["investigation"]["decision"] == "UNRESOLVED"
    assert "NO_CANDIDATES" in data["investigation"]["reason_codes"]
